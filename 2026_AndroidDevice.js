...
(function script () {
  ...
  var schemes = [
  // 0 — RED
  { name: 'RED ', BG:'#1a0000', BG2:'#2a0000', DIM:'#a80000', MAIN:'#ff3333', WARN:'#ff7777', GORE:'#ffaaaa' },  // 2 — PURPLE
  { name: 'UV ', BG:'#14001a', BG2:'#22002a', DIM:'#9b4dff', MAIN:'#c77dff', WARN:'#ff3333', GORE:'#e0b3ff' },
  { name: 'PHOS', BG:'#001b12', BG2:'#002a1c', DIM:'#00a86b', MAIN:'#00ff88', WARN:'#ff3333', GORE:'#66ffcc' },
//3 — WHITE / MONO
  { name: 'WHT ', BG:'#0f0f0f', BG2:'#1a1a1a', DIM:'#888888', MAIN:'#ffffff', WARN:'#ff3333', GORE:'#cccccc' },
  ...
  // 1 — BLUE
  { name: 'BLUE', BG:'#000b1a', BG2:'#00122a', DIM:'#3366ff', MAIN:'#4da3ff', WARN:'#ff3333', GORE:'#99cfff' },
  ...
  // ====================================================
  // ====================================================
// SCREEN + UI LAYOUT (BOTTOM BARS ARE 2 ROWS TALL)
// ====================================================
var w, h, c;

var UI = {
  BUTTON_Y: 1,
  BUTTON_H: 2,      // top bar = 2 rows

  PAGER_H: 2,       // NEW: pager = 2 rows (big paddles)
  STATUS_H: 2,      // NEW: status = 2 rows (bigger hit area)

  PAGER_Y0: null,   // computed
  STATUS_Y0: null,  // computed
  FOOTER_Y: null,   // computed
...
function computeLayout() {
  ...
  // status sits above footer, 2 rows
  UI.STATUS_Y0 = UI.FOOTER_Y - UI.STATUS_H;

  // pager sits above status, 2 rows
  UI.PAGER_Y0  = UI.STATUS_Y0 - UI.PAGER_H;

  // content starts after top button bar
  UI.CONTENT_Y0 = UI.BUTTON_Y + UI.BUTTON_H;

  // content ends right above pager
  UI.CONTENT_Y1 = UI.PAGER_Y0 - 1;
}
  // ====================================================
  // STATE
  // ====================================================
...
UI.TOP_RESERVED = UI.CONTENT_Y0;
  ...
  // TOP BAR (2 rows)
  if (y >= UI.BUTTON_Y && y < (UI.BUTTON_Y + UI.BUTTON_H)) {
    buttonBar.onTouch(x, y);
    return;
  }

  // PAGER (2 rows) - only in apps view
  if (state.view === 'apps' && y >= UI.PAGER_Y0 && y < (UI.PAGER_Y0 + UI.PAGER_H)) {
    pager.onTouch(x, y);
    return;
  }

  // STATUS (2 rows)
  if (y >= UI.STATUS_Y0 && y < (UI.STATUS_Y0 + UI.STATUS_H)) {
...
var buttonBar = {
  ...
  draw: function () {
    ...
    print(c, UI.BUTTON_Y, 'RGB:' + tag + '  ', COL_DIM);
...
// ====================================================
// PAGER (2 ROWS) - BIG PADDLES
// ====================================================
// ====================================================
// PAGER (2 ROWS) - BIG HITBOX, SMALL VISUALS
// ====================================================
var pager = {
  _left: null,
  _right: null,
  ...
  render: function () {
    // clear both pager rows
    background.restoreLine(UI.PAGER_Y0);
    background.restoreLine(UI.PAGER_Y0 + 1);

    // one-line paddles
    var L = '[<]';
    var R = '[>]';

    // draw paddles on TOP row only
    print(1, UI.PAGER_Y0, L, COL_MAIN);

    var rx = Math.max(1, w - R.length - 1);
    print(rx, UI.PAGER_Y0, R, COL_MAIN);

    // centered label across BOTH rows
    var label = 'PAGE ' + (apps.page + 1) + '/' + apps.totalPages + '  (9/PAGE)';
    var lx = Math.max(6, Math.floor((w - label.length) / 2));
    print(lx, UI.PAGER_Y0,     label, COL_DIM);

    // BIG hitboxes: wide + both rows
    pager._left = {
      x0: 0,
      x1: 1 + L.length + 4,
      y0: UI.PAGER_Y0,
  ...
  onTouch: function (x, y) {
    if (!pager._left || !pager._right) return false;

    if (x >= pager._left.x0 && x < pager._left.x1 &&
        y >= pager._left.y0 && y < pager._left.y1) {
    ...
    if (x >= pager._right.x0 && x < pager._right.x1 &&
        y >= pager._right.y0 && y < pager._right.y1) {
